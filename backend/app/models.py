...
class Worker(Base):
    ...
    video_call_requests = relationship("VideoCallRequest", back_populates="worker")
...
class Patient(Base):
    ...
    registered_by_worker_id = Column(String, ForeignKey("workers.worker_id"), nullable=True)
    full_name = Column(String, index=True)
    ...
    video_call_requests = relationship("VideoCallRequest", back_populates="patient")
...
class UserDetail(Base):
    """User/patient table: Aadhaar, name, basic details. History served from Consultation (no photo/audio/referral)."""
    __tablename__ = "user_details"

    aadhaar = Column(String, primary_key=True, index=True)  # 12-char, same as patient_id
    full_name = Column(String, index=True)
    age = Column(Integer, nullable=True)
    gender = Column(String, nullable=True)
    contact_number = Column(String, nullable=True)
...
class User(Base):
    """Portal user: register/login with Aadhaar + password to access dashboard, book appointments, see reminders."""
    __tablename__ = "users"

    aadhaar = Column(String, primary_key=True, index=True)
    password = Column(String)
    full_name = Column(String, index=True)
    contact_number = Column(String, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class VideoCallRequest(Base):
    ...
    patient_id = Column(String, ForeignKey("patients.patient_id"))
    requested_by_worker_id = Column(String, ForeignKey("workers.worker_id"), nullable=True)
    ...
    patient = relationship("Patient", back_populates="video_call_requests")
    worker = relationship("Worker", back_populates="video_call_requests")